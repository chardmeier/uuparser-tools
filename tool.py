...
# A list command
sub_name = 'parser'
uuparser = subparsers.add_parser(sub_name, help='Options for UUParser', parents=[batch_job_options])
train_parse = uuparser.add_mutually_exclusive_group(required=True)
...
# A create command
sub_name = 'tokenizer'
tokenizer = subparsers.add_parser(sub_name, help='Options for UDPipe Tokenizer', parents=[batch_job_options])
train_tokenize = tokenizer.add_mutually_exclusive_group(required=True)
...
# A delete command
sub_name = 'align'
eflomal = subparsers.add_parser(sub_name, help='Options for Eflomal alignment tool', parents=[batch_job_options])
eflomal.add_argument('input', action='store', help='Input for eflomal')
